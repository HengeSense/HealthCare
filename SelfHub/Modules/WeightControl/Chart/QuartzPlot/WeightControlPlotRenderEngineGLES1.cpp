...
private:
    ...
    float FadeValue(float x, float limit, float dist, float y0, float y1, bool isLeftLimit = true);
...
void WeightControlPlotRenderEngineGLES1::Render() {
    ...
    if(tiPerPx>=1800.0 && tiPerPx<4500.0) timeStep*=7;
    ...
    for(plotDataIterator=plotData.begin(),i=0; plotDataIterator!=plotData.end(); plotDataIterator++,i++){
        ...
        //if(tiPerPx<5000){
            weightPointColor.w = FadeValue(tiPerPx, 5000.0, 800.0, 1.0, 0.0, true);
...
float WeightControlPlotRenderEngineGLES1::FadeValue(float x, float limit, float dist, float y0, float y1, bool isLeftLimit){
    float realLimit = isLeftLimit ? limit : (limit+dist);
    
    if(x<=realLimit) return y0;
    if(x>(realLimit+dist)) return y1;
    
    return ((x-realLimit)*(y1-y0) + dist*y0) / dist;
};